...
function validateContactAddress(formData: CheckoutFormData, errors: Record<string, string>) {
...
function validateCardDetails(formData: CheckoutFormData, errors: Record<string, string>) {
  if (!formData.cardNumber?.trim()) {
    errors.cardNumber = "Card number is required";
  } else if (!/^\d{13,19}$/.test(formData.cardNumber.replaceAll(/\s+/g, ""))) {
...
function validateUpiDetails(formData: CheckoutFormData, errors: Record<string, string>) {
...
function validatePayment(formData: CheckoutFormData, paymentGroup: string, isUpi: boolean, errors: Record<string, string>) {
  if (paymentGroup !== "card") return;
  if (isUpi) {
    validateUpiDetails(formData, errors);
  } else {
    validateCardDetails(formData, errors);
  }
}
...
function validateCheckoutForm(
  ...
  validateContactAddress(formData, newErrors);
  validatePayment(formData, paymentGroup, isUpi, newErrors);
...
interface CreateOrderParams {
  ...
  isUpi: boolean;
  displayItems: { productId: string; name: string; price: number; quantity: number; image: string }[];
...
function createOrderData({
  ...
  return {
    ...
    payment: {
      ...
      ...(paymentMethod === "card" && !isUpi && { lastFour: formData.cardNumber?.slice(-4) }),
...
function resolveProductFromUrl(searchParams: URLSearchParams | null): Product | null {
...
function resolveDisplayItems(productFromUrl: Product | null, items: { productId: string; name: string; price: number; quantity: number; image: string; sku: string; slug: string }[], buyNowQuantity: number) {
  if (!productFromUrl) return items;
  return [{
...
function checkHasInsuranceItems(productFromUrl: Product | null, items: { productId: string; name: string; price: number; quantity: number; image: string; sku: string; slug: string }[]): boolean {
...
function resolvePaymentMethod(paymentGroup: string, isUpi: boolean): PaymentMethod {
  if (paymentGroup === "cod") return "cod";
  if (paymentGroup === "insurance") return "insurance";
  if (isUpi) return "upi";
  return "card";
}

// --- Custom Hook to abstract state complexity ---
...
function useCheckoutState() {
  ...
  const productFromUrl = useMemo(() => resolveProductFromUrl(searchParams), [searchParams]);
  const [buyNowQuantity, setBuyNowQuantity] = useState(1);
  
  const displayItems = useMemo(() => resolveDisplayItems(productFromUrl, items, buyNowQuantity), [productFromUrl, buyNowQuantity, items]);
  const displaySubtotal = productFromUrl ? (productFromUrl.price * buyNowQuantity) : subtotal;

  const hasInsuranceItems = useMemo(() => checkHasInsuranceItems(productFromUrl, items), [productFromUrl, items]);
  ...
  const paymentMethod = useMemo(() => resolvePaymentMethod(paymentGroup, isUpi), [paymentGroup, isUpi]);

  const [formData, setFormData] = useState<CheckoutFormData>({
    fullName: "", email: "", phone: "", addressLine1: "", addressLine2: "",
    city: "", state: "", zipCode: "", country: "US", paymentMethod: "cod",
    saveInfo: false, termsAccepted: false, cardNumber: "", cardExpiry: "",
    cardCvv: "", cardName: "", upiId: "",
  });
  ...
  const handleBuyNowQuantityChange = (delta: number) => setBuyNowQuantity((prev) => Math.max(1, prev + delta));
  ...
  const handlePlaceOrder = () => {
    ...
    setTimeout(() => {
      const order = createOrderData({
        formData, paymentMethod, isUpi, displayItems, displaySubtotal, shippingCost, tax, total
      });
      const orderKey = `castor:ecommerce:order:${order.id}`;
  ...
  const handleUpdateQuantity = productFromUrl ? () => {} : updateQuantity;

  return {
    mounted, step, setStep, paymentGroup, setPaymentGroup, isUpi, setIsUpi,
    insuranceFormSubmitted, setInsuranceFormSubmitted, clearCart,
    errors, setErrors, formData, setFormData, displayItems, displaySubtotal,
    hasInsuranceItems, shippingCost, tax, total, buyNowQuantity,
    productFromUrl, paymentMethod, handleBuyNowQuantityChange, handleFormChange,
    handleContinueToReview, handlePlaceOrder, handleRemoveItem, handleUpdateQuantity
  };
}
...
function CheckoutContent() {
  const {
    mounted, step, setStep, paymentGroup, setPaymentGroup, isUpi, setIsUpi,
    errors, formData, setFormData, displayItems, displaySubtotal,
    hasInsuranceItems, shippingCost, tax, total, buyNowQuantity,
    productFromUrl, paymentMethod, handleBuyNowQuantityChange, handleFormChange,
    handleContinueToReview, handlePlaceOrder, handleRemoveItem, handleUpdateQuantity,
    setInsuranceFormSubmitted, clearCart
  } = useCheckoutState();
...
interface CardUpiFormProps {
  isUpi: boolean;
  setIsUpi: (isUpi: boolean) => void;
  formData: CheckoutFormData;
  handleFormChange: (field: keyof CheckoutFormData, value: string | boolean) => void;
  errors: Record<string, string>;
}

function CardUpiForm({ isUpi, setIsUpi, formData, handleFormChange, errors }: CardUpiFormProps) {
  return (
    <motion.div
      initial={{ height: 0, opacity: 0 }}
      animate={{ height: "auto", opacity: 1 }}
            ...
            <div>
              ...
              <input
                ...
                className={`w-full rounded-xl border ${
                  errors.upiId ? "border-red-500" : "border-slate-200"
                } bg-white px-4 py-3 text-[#0e1b33] placeholder:text-slate-400 focus:border-[#20a9ad] focus:outline-none focus:ring-2 focus:ring-[#20a9ad]/20`}
              />
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  maxLength={4}
                  className={`w-full rounded-xl border ${
                    errors.cardCvv ? "border-red-500" : "border-slate-200"
                  } bg-white px-4 py-3 text-[#0e1b33] placeholder:text-slate-400 focus:border-[#20a9ad] focus:outline-none focus:ring-2 focus:ring-[#20a9ad]/20`}
                />
...
interface ReviewOrderStepProps {
  ...
  isUpi: boolean;
  total: number;
  setStep: (step: "form" | "review" | "processing" | "success") => void;
  onPlaceOrder: () => void;
}
